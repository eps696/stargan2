...
class AdainResBlk(nn.Module):
    def __init__(self, dim_in, dim_out, style_dim=64, actv=nn.LeakyReLU(0.2), upsample=False, lowmem=False):
        super().__init__()
        self.actv = actv
...
class Generator(nn.Module):
    def __init__(self, img_size=256, style_dim=64, max_conv_dim=512, lowmem=False): # 1
        super().__init__()
        ...
        for _ in range(repeat_num):
            ...
            self.decode.insert(0, AdainResBlk(dim_out, dim_in, style_dim, upsample=True, lowmem=lowmem))  # stack-like
            dim_in = dim_out

        # bottleneck blocks
        for _ in range(2):
            self.encode.append(ResBlk(dim_out, dim_out, normalize=True))
            self.decode.insert(0, AdainResBlk(dim_out, dim_out, style_dim, lowmem=lowmem))
...
def build_model(args):
    if args.parallel is True:
        generator       = nn.DataParallel(Generator(args.img_size, args.style_dim, lowmem=args.lowmem))
        mapping_network = nn.DataParallel(MappingNetwork(args.latent_dim, args.style_dim, args.num_domains))
        style_encoder   = nn.DataParallel(StyleEncoder(args.img_size, args.style_dim, args.num_domains))
    else:
        generator = Generator(args.img_size, args.style_dim, lowmem=args.lowmem)
